...
NEST_STATES = {0: "off", 1: "heat", 2: "cool", 3: "range", 6: "fan only", 13: "away"}
...
class NestControl(Node):
    ...
    def _discover(self, **kwargs):
        ...
        for device in self.napi.devices:
            ...
            # ISY only allows 14 character limit on nodes, have to strip the RINCON
            ...
            if not lnode:
                ...
                self.parent.thermostats.append(NestThermostat(self.parent, self.parent.get_node('nestcontrol'), 
                                                                  address, self.napi, device.temperature, structure.name,  device.where, manifest))
...
class NestThermostat(Node):
    ...
    def __init__(self, parent, primary, address, napi, temperature, structurename, location, manifest=None):
        self.parent = parent
    ...
    def update_info(self):
        ...
        for device in napi.devices:
            if self.address == device.serial[-14:].lower():
                ...
                self.outsidetemp = nest_utils.c_to_f(napi.structures[0].weather.current.temperature)
                try:
                ...
                self.set_driver('GV4', device.online)
